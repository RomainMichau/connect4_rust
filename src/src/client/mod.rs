    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
    ...
    async fn get_grid(
        &self,
        context: &C) -> Result<GetGridResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::GetGridResponseBody>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GetGridResponse::Status200
                    (body)
                )
            }
            code => {
    ...
    async fn minimax(
        &self,
        param_depth: i32,
        context: &C) -> Result<MinimaxResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/api/solver/minimax",
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::MiniMaxiResponseBody>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(MinimaxResponse::Status200
                    (body)
                )
            }
            code => {
    ...
    async fn post_token(
        &self,
        param_column: i32,
        context: &C) -> Result<PostTokenResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/api/token",
        ...
        let query_string = {
            let mut query_string = form_urlencoded::Serializer::new("".to_owned());
                query_string.append_pair("column",
                    &param_column.to_string());
            query_string.finish()
        };
        if !query_string.is_empty() {
            uri += "?";
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::AddTokenResponseBody>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(PostTokenResponse::Status200
                    (body)
                )
            }
            400 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::BadRequestErrorBody>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(PostTokenResponse::Status400
                    (body)
                )
            }
            code => {
    ...
    async fn reset_game(
        &self,
        context: &C) -> Result<ResetGameResponse, ApiError>
    {
        let mut client_service = self.client_service.clone();
        let mut uri = format!(
            "{}/api/grid/reset",
